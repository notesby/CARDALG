        ...
        for step in range(noSteps):
            ...
            for cell in helpers.multiDimensionalGenerator(size):
                ...
                for op in self.rules:
                    ...
                    if res:
                        xi = cell
                        tmpCell = eval(helpers.getExpr(size))
                        nextStep[tmpCell] = op
...
class Transformer():
    ...
    def getRules(self):
        ...
        for population in self.experiment.world.populations:
            ...
            temp = []
            for key in terms:
                temp.append("x[{}] in {}".format(key,terms[key]))
            lmstr += " and ".join(temp)
...
class ExperimentBaseModel(Experiment):
    ...
    def getProblem(self):
        ...
        backend = 'threading'
        with parallel_backend(backend):
            Parallel(n_jobs=1)(delayed(self.getProblemSub1)(iState,problem,dataSize,stateSize,noStates) for iState in range(1,noStates))
        return problem
