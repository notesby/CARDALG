...
def getLearningProblem(data,target):
    problem = {}
    conflicts = {}
    for row in data:
        cls = row[target]
...
def calculateNoEvents(goalAttributes,domain,dataset):
    ...
    for row in dataset:
        for j,col in enumerate(row[0]):
            noEventsC[j][goalAttributes[0]][row[2]][col] += row[1]
